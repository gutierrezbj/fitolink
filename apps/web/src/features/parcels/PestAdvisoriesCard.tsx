...
const SEV_TONE: Record<Severity, { strip: string; pill: string; dot: string; label: string }> = {
  high:   { strip: 'bg-agrom-alert',   pill: 'bg-agrom-alert/10 text-agrom-alert',     dot: 'bg-agrom-alert',   label: 'severidad alta'  },
  medium: { strip: 'bg-agrom-warning', pill: 'bg-agrom-warning/15 text-agrom-warning', dot: 'bg-agrom-warning', label: 'severidad media' },
  low:    { strip: 'bg-agrom-info',    pill: 'bg-agrom-info/15 text-agrom-info',       dot: 'bg-agrom-info',    label: 'severidad baja'  },
};
...
export default function PestAdvisoriesCard({ parcelId }: Props) {
  ...
  return (
    <div className="bg-white rounded-xl border border-agrom-rule/30 overflow-hidden">
      ...
      {isLoading || !data ? (
        <div className="p-6 text-center text-sm text-agrom-muted">Consultando avisos…</div>
      ) : data.length === 0 ? (
        <div className="px-5 py-8 text-center">
          <p className="font-display text-lg text-agrom-ink mb-1">Sin avisos activos en su zona.</p>
        ...
        <ul>
          {data.map((adv) => {
            ...
            return (
              <li key={adv.advisoryId} className="flex border-t border-agrom-rule/20 first:border-t-0">
                <span className={`w-1 flex-shrink-0 ${tone.strip}`} />
                <div className="flex-1 px-4 py-3">
                  <div className="flex items-baseline justify-between gap-3 flex-wrap mb-1">
                    <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-agrom-deep">
                      <span className="text-agrom-terra">§</span>{' '}
                      {adv.pestName}
                      {adv.scientificName && (
                        <span className="text-agrom-muted italic ml-1 normal-case tracking-normal">
                          ({adv.scientificName})
                        </span>
                      )}
                      <span className="text-agrom-muted mx-2">·</span>
                      <span className="text-agrom-muted">
                        {adv.comarca ? `comarca ${adv.comarca}` : adv.province}
                      </span>
                      <span className="text-agrom-muted mx-2">·</span>
                      <span className="text-agrom-muted">a {adv.distanceKm} km</span>
                    </p>
                  ...
                  {(adv.sourceRef || adv.sourceUrl) && (
                    <p className="font-mono text-[10px] uppercase tracking-wider text-agrom-muted mt-2">
                      {adv.sourceUrl ? (
                        <a href={adv.sourceUrl} target="_blank" rel="noopener noreferrer" className="hover:text-agrom-deep transition-colors underline-offset-2 hover:underline">
