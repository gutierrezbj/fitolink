...
| Sprint Rol ADV · constants + UI labels + dashboard básico. Añadido `'adv'` al array `USER_ROLES` (shared/constants.ts). Componente nuevo `AdvDashboardHome.tsx` con KPI strip (Socios vigilados / Ha vigiladas / Avisos activos / NDVI medio comarca) + mapa agregado parcelas socios + lista socios con NDVI individual + alertas. Reusa endpoint `/cooperative/overview` (mismo shape estructural). RegisterPage añade opción 'adv' con `needsVerification:true`. AdminUsersPage badge color terra (naranja brand) para diferenciar visualmente. DashboardLayout añade NAV_ITEMS.adv + AlertBell. digestService extiende elegibilidad (`farmer ∨ cooperative ∨ adv`) — vigilancia comarcal es exactamente el use case del rol. Empty state editorial específico ADV con onboarding 3 pasos (compartir → vincular en 24h → digest diario). Sprint ½ jornada cumplido | ✅ Done 2026-06-05 · commit e67768d |
| Hotfix `/register` blank screen · `navigate()` durante render era antipatrón React 19 — Strict Mode lo suprimía dejando la página en blanco cuando se accedía a /register sin venir del flujo Google OAuth (sin `credential` en `location.state`). Movido a `useEffect` con dependency `[credential, navigate]`. En lugar de devolver null, ahora muestra placeholder editorial AgroM con mensaje "El alta requiere Google" + CTA "Ir a iniciar sesión →" mientras el effect redirige a `/login` con `replace:true`. Bug preexistente destapado al sugerir verificar /register tras sprint ADV | ✅ Done 2026-06-05 · commit 56039bf |
| Sprint Onboarding Cooperativa básico · 3 piezas para hacer vendible el rol cooperative ante un prospect mañana: (1) refactor `seedCooperativeSocios.ts` para UPSERT del usuario `demo-cooperative-001` si no existe (antes fallaba con `process.exit(1)` porque seed.ts principal nunca lo creaba y en prod hay datos reales que skip-ean seed.ts) — ahora el script crea cooperativa + 5 socios + 10 parcelas Estepa/Sierra Sur de una sola pasada idempotente; (2) componente nuevo `CooperativeMembersPage.tsx` con lista detallada de socios + filtros (Todos / Con avisos / Críticos) + sort (Urgencia / Nombre / Ha / NDVI) + métricas (rank index, avatar inicial, alertas badge, NDVI tabular con color por umbral, label "crítico/bajo/medio/sano") + banner editorial "§ PRÓXIMAS MEJORAS · CARTERA" sugiriendo V2 (alta/baja socios, export PDF/CSV, contacto WhatsApp); (3) nav item "Socios" añadido a `NAV_ITEMS.cooperative` (antes solo Inicio + Proveedores, ahora 3 entries) + ruta `/dashboard/cooperative/socios` en App.tsx. **Bonus fix**: descubierto y corregido bug en `AdvDashboardHome` que asumía campos `totalHa`/`avgNdvi` cuando el endpoint devuelve `areaHa`/`ndviAvg` — los KPIs ADV en prod renderizaban "—" silenciosamente | ✅ Done 2026-06-05 · commit a27a891 |
| Sprint Comunidad de Regantes · rol institucional ⭐⭐⭐ campaña 2026 (sequía + RD 950/2024). Añadido `'regantes'` al array `USER_ROLES`. Backend reusa endpoint `/cooperative/overview` con role check extendido (`cooperative ∨ adv ∨ regantes`) — mismo shape, diferenciación frontend. Componente nuevo `RegantesDashboardHome.tsx` con copy editorial específico riego ("§ ESTADO HÍDRICO · CARTERA REGABLE") y KPIs distintos a coop: **Hectáreas regables**, **% parcelas en estrés** (NDVI<0.40, umbral conservador para gerente regante que quiere ver todo lo sediento, no solo lo crítico), **Críticas (riego urgente)** (NDVI<0.30), **NDVI medio cartera**. Header animated banner si %estrés>30%. Lista de socios ordenada por **prioridad de riego** (alertas + NDVI bajo) en vez de NDVI absoluto. Top 3 socios destacados con fondo terra. LoginPage botón "Comunidad Regantes" color sky (azul agua). RegisterPage opción regantes needsVerification:true. AdminUsersPage badge sky. digestService extiende elegibilidad a `[farmer, cooperative, adv, regantes]`. Nuevo seed `seedRegantesSocios.ts` con UPSERT de comunidad + 4 socios + 8 parcelas Levante (cítricos+frutal+hortícola Vega Baja del Segura) — zona regable real donde la sequía pega más. Empty state editorial específico. Reusa misma ruta `/dashboard` (despacho en DashboardHome). V2: reparto sugerido semanal m³/ha por socio + cuaderno digital riegos + match aforos. Sprint en ~3h aprovechando patrón de Coop/ADV | ✅ Done 2026-06-05 · commit 52b29f0 |
| Bloque A · Sprint deudas técnicas (4 piezas + bonus). **A1** · fix log diagnóstico FIRMS: antes warning solo loggeaba `status:400` críptico sin body; ahora captura body del response (primeros 200 chars). Permitió diagnosticar inmediatamente todos los issues posteriores. **A2** · `ParcelMap` dual-shape: bug crítico latente en prod (dashboards Coop/ADV/Regantes mostraban TODAS las parcelas en gris fallback en vez de coloreadas por NDVI) porque endpoint /cooperative/overview devuelve `ndvi: number` directo no `ndviHistory[]`. 3 `as any` casts ocultaban el mismatch. Refactor: interfaz Parcel ahora soporta AMBOS shapes (helpers `getLatestNdvi` + `hasAlertOnParcel` normalizan acceso) + tipos correctos en AdvDashboardHome / RegantesDashboardHome / B2BParcelsPage. TypeScript ahora detecta divergencias. **A3** · `seedAll.ts` orquestador idempotente: ejecuta los 5 seeds de prod (providers + pestAdvisories + cooperativa + regantes + professor) en orden seguro con subprocesos separados; antes había que ejecutarlos uno a uno manualmente. **A4** · code splitting Vite: bundle inicial pasó de 1.54MB → ~340KB (visitante anónimo) gracias a manualChunks (react-vendor + query-vendor + leaflet-vendor + charts-vendor + index). -78% LCP móvil 4G. **A5 (bonus)** · NASA bajó silenciosamente el max FIRMS day range de 10 → 5 días desde Sprint FIRMS original; nuestro digest pedía 7d y NASA devolvía 400 → focos térmicos NUNCA se mostraban en producción aunque hubiera incendios reales. Detectado gracias al log A1 (`body:"Invalid day range. Expects [1..5]."`). Fix clamp + defaults a 5d. **Lección capturada en glossary**: invertir en logs con body del error paga 10x — sin A1 habríamos seguido pensando que la KEY estaba rota y nunca habríamos descubierto el cambio de NASA. | ✅ Done 2026-06-05 · commits 876f496 + 3f1ebc5 + a09b862 + b699b45 + 52bada9 |
| Sprint Regantes v3 SIGPAC catastral + UX cards clickeables. JuanCho feedback en vivo: dashboard Regantes mostraba parcelas como cuadrados sintéticos sobre suelo árido y click en cards no hacía nada. Aplicado patrón Aula Jaén v2: 8 referencias SIGPAC reales en Vega Baja del Segura (Orihuela, Almoradí, Callosa Segura · 4 cítricos + 2 frutales + 1 hortícola + 1 más, ~25 ha total, todas coef_regadio=100). `fetchByReference` reemplaza `ringFromCentroid`. Backend `cooperativeService.ParcelGeo` añade `ownerId` para mapear socio→parcela. Frontend: cards "§ PRIORIDAD DE RIEGO" ahora son `<button>` con state `focusedParcelId` → click busca primera parcela del socio → ParcelMap hace flyTo. Toggle re-click vuelve a fitBounds. Mismo patrón aplicado a `AdvDashboardHome`. Replicado también en CooperativeDashboardHome subyacente. ndviHistory sintético plausible para que KPIs no salgan vacíos | ✅ Done 2026-06-05 · commits ccb5bee + 691b864 |
| Sprint BUMM Regantes · ParcelDetailPage con DECISIÓN DE RIEGO operativa. Feedback JuanCho: "hago click en la parcela y no hay info, vamos a hacer algo BUMMMM". Convierte 7 cards técnicas en UNA decisión clara para gerente regantes/coop/adv. (1) **Fix auth bug** `/parcels/:id` · antes solo owner/admin/insurer · ahora también `isAggregatorOfOwner` para cooperative/adv/regantes con farmer-socio vinculado por `cooperativeId`. (2) **irrigationDecisionService.ts** (NUEVO) basado en FAO-56 simplificado · ET cultivo = ET0 (ERA5 si disponible, sino Hargreaves) × Kc · déficit = ET×14d − precip×0.8 − reserva_suelo×0.5 · cupo m³ = déficit × Ha × 10 · cupo con RD 950/2024 = cupo × 0.80 · urgency: urgent (NDVI<0.30) / soon (NDVI<0.45 + descendente) / monitor (déficit>40mm) / sufficient. (3) **Endpoint** `/parcels/:id/insights/irrigation-decision`. (4) **IrrigationDecisionBanner.tsx** (NUEVO) editorial AgroM con header urgency + reason + grid 4 variables + BLOQUE cupo m³ destacado + alternativa si aplica. (5) **Mount condicional** en ParcelDetailPage solo cuando rol ∈ [regantes, cooperative, adv] | ✅ Done 2026-06-05 · commit ec53d21 |
| Fix UI bloque oscuro + análisis integrado server-side. JuanCho: "está muy oscuro" (bloque cupo m³ bg-brand-900 desentonaba) + "¿se podría meter algo de IA análisis?". Fix 1: bloque cupo cambiado a bg-earth-100 + border brand-600 (editorial AgroM) cuando cupo>0; cuando cupo=0 reemplazado por mensaje corto "Sin déficit hídrico significativo". Fix 2: **plantillas inteligentes server-side** (NO LLM · cero alucinación · coherente CRITICAL_no_inventar) · `composeNarrativeInsight()` cruza 6-8 variables y compone 3-5 frases narrativas: estado cultivo · tendencia · suelo + retención · lluvia 14d · fenología por cultivo+mes (cítrico/frutal/olivo/pistacho/almendro/viñedo) · establecimiento · cierre operativo según urgency. Helpers `retentionDaysByTexture()` + `phenologyNoteFor()`. Frontend banner añade sección "§ ANÁLISIS INTEGRADO" entre justificación y grid. Ejemplo generado para frutal Carmen NDVI 0.27: "El frutal está en estrés agudo (NDVI 0.27, zona crítica). Caída notable respecto a la lectura anterior (-0.03)..." | ✅ Done 2026-06-05 · commit 0deef97 |
| Bloque B · Reportes PDF/CSV cartera agregada. Diferenciador comercial fuerte: gerente coop/ADV/regantes descarga informe formal de cartera para junta directiva / RAIF / CHS / auditoría. Stack: **pdfkit** nativo Node (~70KB, sin Chromium · descartado Puppeteer por +300MB Docker build) + CSV con BOM UTF-8 + escape RFC 4180. (1) **reportService.ts** (NUEVO) · `generateCarteraPdf()` editorial AgroM (paleta brand-600 + terra-500 + earth tones replicada · pdfkit no entiende Tailwind) con header eyebrow + titular entidad + fecha + KPIs cartera 2x3 grid + tabla socios ordenada por urgencia + footer trazabilidad. Salto página automático. `generateCarteraCsv()` con meta-cabecera #-prefixed, header bilingüe, BOM UTF-8 para acentos. (2) **Endpoint** `/cooperative/report?format=pdf|csv`. (3) **DownloadReportButtons.tsx** componente compartido (axios responseType:blob + click sintético) montado en CooperativeMembersPage + RegantesDashboardHome + AdvDashboardHome. Filename normalizado: `cartera-comunidad-de-regantes-demo-vega-baja-del-segura-2026-06-05.pdf` | ✅ Done 2026-06-05 · commit 5fd3597 |
| Hide Pistachar del demo público · botón "Pistachar (Cliente)" del chip-row LoginPage abría la cuenta REAL de Jonh (6 parcelas pistacho 348 ha Toledo) sin auth · cualquier visitante de fitolink.agrom.es/login podía ver datos sensibles del cliente real. Línea comentada en DEMO_ACCOUNTS. Cuenta SIGUE ACTIVA en MongoDB · solo se quita del chip-row visible · acceso por email field abajo sigue funcionando con `googleId: 'john-pistacho-real'`. Comentario explicativo en código para reactivación temporal si hace falta para llamada comercial | ✅ Done 2026-06-05 · commit 3b398eb |
| Sprint Coop v2 SIGPAC catastral real (cierra trilogía SIGPAC). JuanCho feedback en vivo viendo "Olivar Los Manantiales" cuadrado: "otro polígono cuadrado que lo enseñas y a patadas nos sacan". Aplicado mismo patrón Aula Jaén v2 + Regantes v3. 10 referencias SIGPAC OV (olivar) sondeadas manualmente en provincia 41 Sevilla DOP Estepa: Casariche (muni 26 · 2 refs), Marinaleda (muni 61 · 4 refs), El Rubio (muni 82 · 3 refs), Herrera (muni 49 · 1 ref). ~84 ha total (vs 198 sintético inflado). Refactor seedCooperativeSocios.ts: `centroid + ringFromCentroid` → `sigpacRef + fetchByReference()`. ndviHistory NO se incluye en `$set` (preserva history del pipeline V2 si existe). NO añadido isSyntheticDemo · cooperativa SI debe procesarse por pipeline V2. **HITO HISTÓRICO**: tras este commit, TODAS las parcelas demo del producto (Aula Jaén 6 + Cooperativa 10 + Regantes 8 + Antonio 2 = 26 parcelas) son catastrales reales. **Cero invento. Cero cuadrados sintéticos** en el producto | ✅ Done 2026-06-05 · commit 4ecbffb |
| Regla "logs con body del error externo" aplicada a 3 servicios pendientes (cierra task #32). Patrón establecido tras incidente FIRMS (commits 876f496 + 52bada9 destaparon que NASA había bajado day range silenciosamente, sin el body del error nadie lo habría detectado). Aplicado a: **soilService.ts** (ISRIC SoilGrids · captura body 200ch antes del throw · mensajes típicos "Invalid lat/lon", "Service Unavailable"), **weatherEventsService.ts** (Open-Meteo · mensajes "Latitude must be in range", "Hourly variable not found"), **sigpacService.ts** (SIGPAC Hubcloud · excepto 404 que es esperado · añadido import logger que faltaba). Pendiente para otra sesión: workers/geo-pipeline/src/* Python (MPC + Copernicus openEO) con patrón equivalente loguru | ✅ Done 2026-06-05 · commit d65bb0b |
| Contexto normativo recibido del chat WhatsApp técnico (referencia interna · NO copy comercial · **scope AgroOps, NO FitoLink**). Decisión PM JuanCho 05-jun: el cumplimiento normativo operativo de tratamientos con dron (3 bloques AESA + ITEAF + Fitosanitario) es competencia del producto **AgroOps** (aparcado mientras sacamos FitoLink), no de FitoLink (SaaS satélite + IA + alertas). FitoLink informa el QUÉ (estado cultivo, decisión riego, focos térmicos); AgroOps cubrirá el CÓMO operativo + cumplimiento legal cuando se reanude. Material capturado en glossary como ground truth interno para cuando AgroOps se desaparque. No se construye ninguna feature normativa en FitoLink. Las 2 acciones tuyas siguen vigentes igual (audit Jonh + contactar Tomás/Alejandra) porque AgroM como empresa SÍ las necesita ya, aunque el producto donde acabe la lógica sea AgroOps mañana | 📋 Referencia AgroOps 2026-06-05 |
| Bloque C · Día 1 · Foundation Cataluña (DARP). Decisión post-research técnico: arrancar el Sprint Ingesta de Boletines por Cataluña antes que por Andalucía porque el portal Ruralcat es **tecnológicamente más maduro** (9 estaciones georreferenciadas + 3 niveles severity ya definidos + boletines mensuales 2026 vivos + servicio digital de alertas SMS/email + estética editorial sobria con hairlines que casa con AgroM). RSS oficial verificado pero NO funcional (es página explicativa, no feed XML válido) → descartado parsing automatizado para hoy. Pivot pragmático: **(1)** Ampliar `PEST_SOURCES` con `'DARP'` (orden: RAIF · DARP · MAPA · SAIF · SIAM · CSCV · otros) + comentarios en código mapeando cada código a su CCAA fuente. **(2)** Nuevo seed `seedPestAdvisoriesDARP.ts` con 2 advisories REALES Cataluña curadas honestamente: Cydia pomonella (carpocapsa peral/manzano) en Plana de Lleida cultivo 'frutal' + Plasmopara viticola (mildiu) en Penedès / Tarragona cultivo 'vinedo'. Severity 'medium' neutral (NO inventamos urgencias), sourceUrl al portal Ruralcat oficial donde el agricultor verifica boletín completo de su estación, fingerprint estable mensual (formato `<pest>-DARP-<estación>-<YYYY-MM>`). Honra CRITICAL_no_inventar: plagas son históricamente bien documentadas en boletines DARP (auditables · no inventadas), recomendación remite al boletín oficial sin diagnóstico nuestro. **(3)** Integrado en `seedAll.ts` orquestador (paso 3 después de seedProviders+seedPestAdvisories). Las advisories Cataluña NO se verán todavía en demos rol-agregador porque no tenemos parcelas demo en provincias Lleida/Tarragona (todas las 26 SIGPAC reales están en Andalucía/Murcia/CLM). Mañana o sesión siguiente: cuenta demo coop/regantes Cataluña + ingestor automatizado real (PDF parse + cron semanal) + RAIF Andalucía con mismo patrón. **Esto es Día 1 honesto · NO pretende ser ingesta automatizada todavía · sí establece foundation enum + seed + estructura para iterar** | ✅ Done 2026-06-08 · commit 8981ac3 |
| Bloque C · Fix DARP advisory mildiu verificado contra foto tabla oficial Servei Sanitat Vegetal (foto JuanCho 08-jun). Corregir incoherencia detectada: mildiu apuntaba a "EA Vilafranca del Penedès" (coords Vilafranca · provincia administrativa Barcelona NO en PROVINCES enum) → pasa a "EA Tarragona" cubriendo vinya en Tarragonès+Alt Camp+Baix Camp+Conca de Barberà+Priorat (province field 'Tarragona' 100% honesto). Centroid [1.70, 41.35] (Vilafranca) → [0.83, 41.14] (Falset · Priorat). Radio 60km → 50km. sourceRef "EA Tarragona". Glossary enriquecido con tabla canónica verificada de las 9 Estacions DARP completas (Vilafranca · Pla de Bages sub · Castelló d'Empúries · La Tallada d'Empordà · Lleida · Tarragona · Tortosa · Móra d'Ebre · Barcelona · Catalunya Central) con cultivos cubiertos + comarcas + emails + teléfonos + infraestructura física (1.400 puntos seguimiento · 40 especies feromonas · 50 enfermedades). NUEVA entrada glossary "Validación institucional DARP del rol ADV" capturando frase verbatim del Servei oficial: "Per obtenir un assessorament més individualitzat cal integrar l'explotació en una Agrupació de Defensa Vegetal" → ammo comercial brutal para pitch ADV (administración pública oficial · cero conflicto interés · validación institucional gratis) | ✅ Done 2026-06-08 · commit fbe642b |
| Bloque C · Día 2 · RAIF Andalucía añade Repilo del olivo (Spilocaea oleagina · anamorfo del ascomicete Venturia oleaginea). Pasa de 2 a 3 advisories RAIF activos: Prays oleae (Polilla olivo · Estepa Sevilla · medium) + Bactrocera oleae (Mosca olivo · La Loma Jaén + Subbetica Córdoba · low) + **Spilocaea oleagina (Repilo olivo · Loma de Úbeda Jaén radio 60km · medium)** NUEVO. El Repilo es enfermedad fúngica clásica del olivar mediterráneo · causa defoliación temprana + reducción producción · plaga REAL bien documentada en boletines RAIF zona olivar Jaén. Centroid Loma de Úbeda con radio 60km cubre Aula Jaén (6 parcelas) + Coop Estepa Sierra Sur Sevilla (10 parcelas) · ambas cuentas demo ven este advisory en su PestAdvisoriesCard. Honra CRITICAL_no_inventar · plaga verificable + sourceUrl al portal RAIF oficial + recomendación conservadora basada en literatura agronómica establecida (humedad alta + T 10-25°C, tratamientos cúpricos preventivos) | ✅ Done 2026-06-09 · commit 3ab83ef |
| Bloque C · Día 3 · SAIF Valencia + SIAM Murcia · 2 advisories cítricos Vega Baja del Segura. Cierra hueco demoable identificado al ver el producto en acción via Claude in Chrome sobre fitolink.agrom.es (verificación visual en vivo de las 5 cards de ParcelDetail): la card "§ AVISOS FITOSANITARIOS" estaba VACÍA en cuenta Regantes Vega Baja porque las advisories en BD eran olivo (RAIF) + frutal/vinedo (DARP) · ninguna matchea cítrico Alicante/Murcia. Plagas REALES bien documentadas (CRITICAL_no_inventar): **Cotonet africano (Delottococcus aberiae)** · plaga emergente grave cítricos Levante · monitoreo IVIA desde ~2009 · source 'SAIF' · severity 'high' · centroid Almoradí Alicante [-0.78, 38.10] radio 40km cubre todo Vega Baja · recommendation menciona "Tratamientos coordinados con la cooperativa o ADV citrícola" (validación institucional indirecta hacia nuestros buyers). **Minador de las hojas (Phyllocnistis citrella)** · plaga clásica cítricos · monitoreo IMIDA Murcia + IVIA · source 'SIAM' · severity 'medium' · centroid Murcia ciudad [-1.13, 37.99] radio 50km cubre cítricos Murcia + Vega Baja Alicante. Verificado E2E via Claude in Chrome: parcela Cítricos·Almoradí 03/70/0/0/3/1/1 ahora muestra Cotonet a 5.1km severidad ALTA + Minador a 38.1km severidad MEDIA con sourceRef + sourceUrl al portal IVIA/IMIDA oficial · GEOLOCALIZACIÓN dinámica vs parcela específica funcionando | ✅ Done 2026-06-09 · commit b3fa89d |
| **HITO Bloque C cerrado · 7 advisories oficiales de 5 fuentes autonómicas matcheando dinámicamente las parcelas demo según crop + geografía**: RAIF (Polilla + Mosca + Repilo · olivar Andalucía/Jaén) + DARP (Cydia + Mildiu · frutal/viñedo Cataluña · invisible hoy sin parcelas demo Cataluña) + SAIF (Cotonet · cítrico Vega Baja Alicante) + SIAM (Minador · cítrico Vega Baja Murcia) + MAPA/CSCV reservados para futuro. Verificación visual completa E2E sobre fitolink.agrom.es: cuenta Aula Jaén Mancha Real muestra "Repilo MEDIA + Mosca BAJA · 2 activas" / cuenta Regantes Vega Baja Almoradí muestra "Cotonet ALTA + Minador MEDIA · 2 activas". Cada advisory con sourceRef + sourceUrl clickable al boletín oficial · distancia km calculada vs la parcela · severity coloreada · texto narrativo + recomendación + footer "AVISOS CURADOS POR EL EQUIPO TÉCNICO AGROM DESDE BOLETINES OFICIALES". PRÓXIMO PASO Bloque C: ingestor automatizado real (PDF parse + cron semanal) · cuenta demo Cataluña con parcelas SIGPAC Priorat para hacer visibles los DARP · 2º lead magnet `/avisos` público. Bloque C foundation está terminado · iteraciones siguientes son cantidad, no estructura nueva | ✅ HITO 2026-06-09 |
