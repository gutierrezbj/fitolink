...
| Demo Aula Jaén Jesús (cuenta pedagógica): seed idempotente `seedProfessorDemo.ts` crea usuario `jesus-vivar-edu` (Jesús Vivar, jesusvivar22@gmail.com) con 6 parcelas sintéticas Jaén ilustrando escenarios canónicos de agricultura precisión (olivar sano, intensivo seto, pistachar calibrando sin plantingYear, almendro establecimiento informado, olivar estrés hídrico con alerta crítica activa + Operation completada, olivar heterogéneo). Nuevo flag `Parcel.isSyntheticDemo: boolean` → pipeline V2 + build_mpc_baselines.py ignoran estas parcelas (no sobrescriben datos hand-crafted). Botón demo "Aula Jaén Jesús" en LoginPage. Para clases del profesor en su curso UPM-Jaén. | ✅ Done 2026-05-18 |
