...
| ⚠️ INCIDENTE prod offline 14-may: durante manipulación DNS de `agrom.es` en Hostinger se borró por accidente el A record `fitolink → 187.77.71.102` en la zona `systemrapid.io` (mismo panel, fácil cruzar dominios). Resultado: `fitolink.systemrapid.io` NXDOMAIN, producción inaccesible. Servidor 187.77.71.102 sigue arriba con bundle nuevo (commit a91383d) desplegado. **Acción pendiente antes de viajar**: panel Hostinger → systemrapid.io → DNS Records → Add `A · fitolink · 187.77.71.102 · TTL 300` | 🔴 Pendiente 2026-05-14 |
